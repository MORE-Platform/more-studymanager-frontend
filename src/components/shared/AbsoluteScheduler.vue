...
<script setup lang="ts">
  ...
  console.error('absolute scheduler');
  console.log(scheduler.rrule);
  console.log(returnSchedule.value.rrule);
  ...
  calendarEnd.value.setDate(calendarEnd.value.getDay() + 1);
  ...
  const entireDayCheckbox: Ref<boolean> = ref(
    calendarStart.value.getHours() === calendarEnd.value.getHours() &&
      calendarStart.value.getMinutes() === calendarEnd.value.getMinutes() &&
      calendarStart.value.getSeconds() === calendarEnd.value.getSeconds()
      ? true
      : false
  );
  ...
  function onChangeSingleDayEventCheckbox() {
    if (
      ...
      const oldDateTime = calendarEnd.value;

      if (calendarStart.value.getDate() !== calendarEnd.value.getDate()) {
        calendarEnd.value.setDate(calendarStart.value.getDate());
        calendarEnd.value.setTime(oldDateTime.getTime());
      }
    }
  }
