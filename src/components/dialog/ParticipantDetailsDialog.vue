...
<script setup lang="ts">
  ...
  import {
    OccurredObservation,
    OccurredObservationStateEnum,
    Participant,
    ParticipantDataHealthIndicatorEnum,
  } from '@gs';
  ...
  const dataHealthIndicatorBtn = computed(() => {
    if (participant.dataHealthIndicator) {
      switch (participant.dataHealthIndicator) {
        case ParticipantDataHealthIndicatorEnum.Green:
          return 'btn-accepted';
        case ParticipantDataHealthIndicatorEnum.Orange:
          return 'btn-warn';
        case ParticipantDataHealthIndicatorEnum.Red:
  ...
  function getDataHealthIndicatorColor(
    state: OccurredObservationStateEnum,
  ): string {
    switch (state) {
      case OccurredObservationStateEnum.Completed:
        return 'bg-[var(--green-500)]';
      default:
        return 'bg-[var(--orange-500)]';
    }
  }
  function getDataHealthIcon(state: OccurredObservationStateEnum): string {
    switch (state) {
      case OccurredObservationStateEnum.Completed:
        return 'pi pi-check';
      default:
                ...
                >
                  <span
                    :class="[
                      ...
                      'flex aspect-square items-center justify-center px-2 py-2',
                    ]"
                  >
                    <span
                  ...
                  <span>{{
                    `${$t(`study.dataHealth.labels.${data.healthState}`)}`
                  }}</span>
                </span>
