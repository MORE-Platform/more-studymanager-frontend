<script setup lang="ts">
  ...
  const customPrompt: Ref<string> = ref('');
  const sanitizedOutput: Ref<string> = ref('');
  ...
  function updateSanitizedOutput() {
    if (outputContainer.value) {
      sanitizedOutput.value = DOMPurify.sanitize(customPrompt.value);
    }
  }

  watch(customPrompt, updateSanitizedOutput);
</script>
  ...
  <div>
    <div class="title mb-8">
      <h3 class="mb-1 font-bold">
        {{ $t('data.AISenseMaking.title') }}
      </h3>
      <div>
        {{ $t('data.AISenseMaking.filterForAI') }}
      </div>
    </div>
    <div class="flex justify-between">
      <h3 class="mb-1 font-bold">{{ $t('data.AISenseMaking.filterForAI') }}</h3>
      <Button
    ...
    <!-- Main content area with prompt and output -->
    <div class="grid grid-cols-2 gap-4 mt-4">
      <!-- Left column: Custom Prompt Section -->
      <div>
        <h3 class="mb-1 font-bold">{{ $t('data.AISenseMaking.customPrompt') }}</h3>
        <div class="text-sm text-gray-600 mb-2">
          {{ $t('data.AISenseMaking.customPromptDescription') }}
        </div>
        <textarea
          v-model="customPrompt"
          class="w-full p-2 border rounded-md"
          rows="4"
          :placeholder="$t('data.AISenseMaking.customPromptDescription')"
        ></textarea>
      </div>

      <!-- Right column: Output Canvas -->
      <div class="border rounded-md p-4 bg-white min-h-[300px]" ref="outputContainer">
        <div class="h-full w-full" v-html="sanitizedOutput"></div>
      </div>
    </div>
