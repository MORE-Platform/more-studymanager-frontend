...
<script setup lang="ts">
  ...
  const rowActions: MoreTableAction[] = [
    {
      ...
      confirmDeleteDialog: {
        header: t('interventionToken.dialog.header.delete'),
        message: t('interventionToken.dialog.deleteMsg.intro'),
        dialog: (row: any) =>
          dialog.open(DeleteMoreTableRowDialog, {
            data: {
              introMsg: t('interventionToken.dialog.deleteMsg.intro'),
              warningMsg: t('interventionToken.dialog.deleteMsg.warning'),
              confirmMsg: t('interventionToken.dialog.deleteMsg.confirm'),
              row: row,
              elTitle: `${row.tokenLabel} (Intervention: ${row.interventionTitle})`,
            },
            props: {
              header: t('interventionToken.dialog.header.delete'),
              style: { width: '50vw' },
            ...
            onClose: (options) => {
              if (options?.data) {
                executeAction({ id: 'delete', row: options.data } as MoreTableRowActionResult);
              }
            },
          }),
      },
  ...
  async function openTokenDialog(): Promise<void> {
  ...
  async function createToken(data: {
    ...
    try {
      const response = await axios.post(
        ...
        { tokenId: 0, tokenLabel: data.tokenLabel, created: '', token: '' },
      );
  ...
  function openInfoDialog(token: any): void {
  ...
  async function deleteToken(row: InterventionTokenTableMap): Promise<void> {
    try {
      ...
      handleIndividualError(e, `Cannot delete token: ${row.tokenId}`);
    }
  }
...
<template>
  <div class="intervention-token-list">
    <MoreTable
      row-id="tokenId"
